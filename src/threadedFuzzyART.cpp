...
ThreadedFuzzyART::ThreadedFuzzyART(int input_dim, int max_categories, float vigilance, float choice_alpha, float learning_rate, int init_categories, int num_threads)
    : input_dim_(input_dim), max_categories_(max_categories),
      vigilance_(vigilance), choice_alpha_(choice_alpha), learning_rate_(learning_rate), num_threads_(num_threads)
{
...
int ThreadedFuzzyART::run(const std::vector<float> &input)
{
    ...
    // Threaded computation of choice function
#pragma omp parallel for schedule(static) num_threads(num_threads_)
    for (int j = 0; j < num_initialized_categories_; j++)
    {
        ...
        {
            float w = weights_[offset + k];
            norm_intersection += std::min(I[k], w);
            norm_w += w;
        }

        float match_score = norm_intersection / (choice_alpha_ + norm_w);
        candidates[j] = {match_score, j};
    }

    // Sort candidates by choice function score in descending order
    std::sort(candidates.begin(), candidates.end(), [](const auto &a, const auto &b)
              { return a.first > b.first; });

    // Resonance check and learning
    for (const auto &candidate : candidates)
    {
        ...
        float match_score = norm_intersection / (float)input_dim_;
